...
class Torch_model:
    ...
    def warmup(self, img_size=None):
        ...
        logger.info(f"Warming up model with input size {img_size}...")
        dummy_input = np.zeros((img_size[1], img_size[0], 3), dtype=np.uint8)
    ...
    def _load_model(self):
        ...
        # specific for D-FINE: anchors and valid_mask depend on img_size
        # We must remove them if we are running at a different resolution than training
        model_state = self.model.state_dict()
        keys_to_remove = ["decoder.anchors", "decoder.valid_mask"]
        
        for k in keys_to_remove:
            if k in ckpt and k in model_state:
                if ckpt[k].shape != model_state[k].shape:
                    logger.info(f"Removing {k} from checkpoint due to shape mismatch (Resize detected).")
                    del ckpt[k]
        
        # Use strict=False because we might have removed keys
