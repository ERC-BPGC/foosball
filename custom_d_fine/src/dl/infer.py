...
def run_webcam(torch_model, output_path, label_to_name, to_crop, paddings, webcam_id=0):
    batch = 0
    labels = set()
    
    vid = cv2.VideoCapture(webcam_id)
    if not vid.isOpened():
        logger.error(f"Cannot open webcam {webcam_id}")
        return

    idx = 0
    
    pbar = tqdm(desc="Webcam Inference (Press Ctrl+C to stop)")
    ...
    try:
        while True:
            ...
            frame_name = f"webcam_frame_{idx}"
            ...
            if to_crop:
                crops(img, res, paddings, "webcam", output_path, frame_name)
...
def main(cfg: DictConfig):
    ...
    if cfg.infer.source == "webcam":
        run_webcam(
            ...
            to_crop=to_crop,
            paddings=paddings,
            webcam_id=cfg.infer.webcam_id,
        )
    else:
        # Default to folder inference
