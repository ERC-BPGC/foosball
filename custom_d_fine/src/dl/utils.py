...
def get_latest_experiment_name(exp: str, output_dir: str):
    ...
    if latest_exp is None:
        final_exp_name = f"{target_exp_name}_{datetime.now().strftime('%Y-%m-%d')}"
    else:
...
def calculate_iou(boxA: np.ndarray, boxB: np.ndarray) -> float:
    ...
    Args:
        boxA (np.ndarray): The first bounding box, format [x1, y1, x2, y2].
        boxB (np.ndarray): The second bounding box, format [x1, y1, x2, y2].
    ...
    # The max(0,...) ensures that if the boxes do not overlap, the area is 0.
